...
// Recover User Password (Forgot Password via backend Brevo API with oobCode extraction)
export const recoverUserPassword = async (email) => {
  if (!email) throw new Error("El correo es requerido.");
  console.log(`[FRONTEND DISPARO] Solicitando restablecimiento de contraseña vía backend (/api/send-otp) para: ${email}`);
  ...
  try {
    ...
    console.error("Error al solicitar correo de restablecimiento vía backend:", error);
    throw error;
  }
};
