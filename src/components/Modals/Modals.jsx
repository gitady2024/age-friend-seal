...
import { buildDossierHtml, downloadTextFile, downloadPitchB2BPdf, generateDecalSvg, generateSvgMatrix } from "../../utils/downloads.js";
...
function Modals({ language, activeModal, contactLevel, currentUser, latestDiagnostic, prefillEmail, resetOobCode, onClose, onOpenAuth, onOpenAccount, onOpenOtp, onUserChange, directPitchDownload, onClearDirectPitch, onOpenPitchSuccess, onOpenPitch }) {
  ...
  useEffect(() => {
    if (directPitchDownload) {
      // Sync lead to Brevo for logged-in users (all languages)
      if (currentUser && currentUser.email) {
        ...
        if (currentUser.userType === 'empresa' || currentUser.companyName || currentUser.economicSector || currentUser.sector) {
          userTypeStr = "Empresa";
        } else if (currentUser.type !== 'anonymous') {
        ...
        const currentLang = String(language || 'ES').toUpperCase();
        ...
        }).catch(err => console.error("Error capturing B2B lead (directPitch):", err));
      }
  ...
  }, [directPitchDownload, language, currentUser, onClearDirectPitch, onOpenPitchSuccess]);
            ...
            {activeTab === 'admin' && (currentUser?.role === 'admin' || currentUser?.email?.toLowerCase().startsWith("admin")) && (
              <div className="portal-tab-content admin-tab" style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <div style={{ marginTop: '20px', background: 'rgba(15, 23, 42, 0.2)', padding: '15px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                  <h5 style={{ margin: '0 0 15px 0', fontSize: '0.95rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    ⚙️ {language === 'es' ? 'Status del Cuestionario de Autodiagnóstico' : 'Self-Diagnostic Questionnaire Status'}
                  </h5>

                  {/* Tabs Selector for Sector & Prominent Export Button */}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: '15px', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>
                    ...
                    >
                      {exportingQuestions ? (
                        <span>{language === 'es' ? 'Exportando Excel...' : 'Exporting Excel...'}</span>
                      ) : (
                        <>📊 {language === 'es' ? 'Exportar Banco de Preguntas (.xlsx)' : 'Export Questions Bank (.xlsx)'}</>
                      )}
                    </button>
                  </div>
